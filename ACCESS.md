...
By default, a DM from an unknown sender triggers **pairing**: the bot replies with a 6-character code and drops the message. You run `/claude-telegram-companion:access pair <code>` from your assistant session to approve them. Once approved, their messages pass through.

All state lives in `~/.claude/channels/telegram/access.json`. The `/claude-telegram-companion:access` skill commands edit this file; the server re-reads it on every inbound message, so changes take effect without a restart. Set `TELEGRAM_ACCESS_MODE=static` to pin config to what was on disk at boot (pairing is unavailable in static mode since it requires runtime writes).
...
| Policy | Behavior |
| --- | --- |
| `pairing` (default) | Reply with a pairing code, drop the message. Approve with `/claude-telegram-companion:access pair <code>`. |
...
```
/claude-telegram-companion:access policy allowlist
```
...
```
/claude-telegram-companion:access allow 412587349
/claude-telegram-companion:access remove 412587349
```
...
```
/claude-telegram-companion:access group add -1001654782309 --no-mention
/claude-telegram-companion:access group add -1001654782309 --allow 412587349,628194073
/claude-telegram-companion:access group rm -1001654782309
```
...
```
/claude-telegram-companion:access set mentionPatterns '["^hey claude\\b", "\\bassistant\\b"]'
```

## Delivery

Configure outbound behavior with `/claude-telegram-companion:access set <key> <value>`.
...
```
/claude-telegram-companion:access set ackReaction 👀
/claude-telegram-companion:access set ackReaction ""
```
...
| Command | Effect |
| --- | --- |
| `/claude-telegram-companion:access` | Print current state: policy, allowlist, pending pairings, enabled groups. |
| `/claude-telegram-companion:access pair a4f91c` | Approve pairing code `a4f91c`. Adds the sender to `allowFrom` and sends a confirmation on Telegram. |
| `/claude-telegram-companion:access deny a4f91c` | Discard a pending code. The sender is not notified. |
| `/claude-telegram-companion:access allow 412587349` | Add a user ID directly. |
| `/claude-telegram-companion:access remove 412587349` | Remove from the allowlist. |
| `/claude-telegram-companion:access policy allowlist` | Set `dmPolicy`. Values: `pairing`, `allowlist`, `disabled`. |
| `/claude-telegram-companion:access group add -1001654782309` | Enable a group. Flags: `--no-mention` (also requires disabling privacy mode), `--allow id1,id2`. |
| `/claude-telegram-companion:access group rm -1001654782309` | Disable a group. |
| `/claude-telegram-companion:access set ackReaction 👀` | Set a config key: `ackReaction`, `replyToMode`, `textChunkLimit`, `chunkMode`, `mentionPatterns`. |
